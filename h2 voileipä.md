...
On tärkeää pitää mielessä että jos sudoa käytetään järjestelmän hallintaan, sen virheellinen konfigurointi voi estää kokonaan pääsyn korjaustoimenpiteisiin normaalin käyttäjän kautta. Tämän riskin ehkäisemiseksi avattiin uusi terminaali-ikkuna ja muodostettiin SSH-yhteys kohdekoneeseen. Tämän jälkeen avattiin erillinen pääkäyttäjän (root) komennolla: sudo -i.
